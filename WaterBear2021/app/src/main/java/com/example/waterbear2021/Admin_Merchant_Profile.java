...
public class Admin_Merchant_Profile extends AppCompatActivity {
    ...
    private Admin_Merchant_Adapter merchant_adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_admin__merchant__profile);

        options = new ArrayList<>();

        recyclerView = findViewById(R.id.merchant_recyclerView);
        ...
        merchantRef.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()){
                    merchantRef.whereEqualTo("Approved","1").orderBy("Store Name").addSnapshotListener(new EventListener<QuerySnapshot>() {
                        @Override
                        public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                            ...
                            merchant_adapter = new Admin_Merchant_Adapter(getApplicationContext(),options);
